...
- `remote/`
  ...
  其中 `transport.py` 定义执行编排使用的 transport 接口，`access.py` 提供当前 SSH/scp 实现，`error_contract.py` 管错误码和状态映射，`executor_preflight.py` 管批量执行前置检查，`script_capabilities.py` 管脚本级目标与 pod 内工具探测，`script_output_contract.py` 管脚本输出合同，`script_runner.py` 管单脚本执行编排，`executor.py` 和 `capabilities.py` 管兼容门面，`cli.py` 管远程执行器 CLI。
- `modes.py`
