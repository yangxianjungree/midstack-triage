...
def blocked_output(context: Dict[str, Any], reason: str) -> Dict[str, Any]:
    started_at = now_iso()
    script_id = str(context.get("script_id") or "pulsar.collect.broker.topic_stats")
    return {
        "script_id": script_id,
        "status": "blocked",
        "summary": reason,
        ...
        "structured_record_patch": {
            "summary": {"broker_topic_stats_status": "blocked"},
            "details": {"broker_topic_stats": {"status": "blocked", "reason": reason}},
        },
        "signal_bundle_patch": {},
        "collection_report_patch": {
            "collection_actions": [{"script_id": script_id, "status": "blocked", "summary": reason}],
            "blank_items": [script_id],
        },
        "warnings": [],
        "evidence_gaps": [
            {
                ...
                "why_important": "Broker topic stats are required to validate backlog growth on the broker side.",
            }
        ],
    }
...
def main() -> int:
    ...
    if not capabilities.get("pulsar_admin_available", capabilities.get("kubectl_available", True)):
        write_data(output_file, blocked_output(context, "pulsar-admin is not available in the provided context"))
        return 0
    write_data(
        output_file,
        blocked_output(context, "live pulsar-admin collection is not implemented in the Pulsar MVP script yet"),
    )
    return 0
