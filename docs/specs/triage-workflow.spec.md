...
`historical_resolved` 当前不进入完整历史故障分析。Phase 2 应先返回
`blocked`，要求用户补充故障时间窗口和历史证据，例如故障窗口内的日志、
Kubernetes events、监控截图/链接或已有 artifact 目录。当前 live collection
只能证明“现在状态”，不得单独用于推断过去故障根因。
...
- `incident_time.mode` 不是 `historical_resolved`，或历史故障已提供可用的
  时间窗口和历史证据路径
...
- 用户线索表明故障已经恢复或发生在过去，但缺少故障时间窗口和历史证据
