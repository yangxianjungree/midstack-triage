...
当前 `/midstack:analyse` 的生产 `analysis.yaml` 由 `src/phases/phase4/rules/<middleware>.py` 的 rules fallback analyzer 生成，并经过 guardrails 修正后用于 `report.md`、`review` 和后续 finalize。

`src/phases/phase4/multitrack/` 是第 4 段多轨推理实现，当前默认仍走 mock / 辅助诊断路径。它会写入 `reasoning-board.yaml` 和 `analysis.multitrack.yaml`，但不写生产 `analysis.yaml`。

`agent-reasoning-task.md` 是人工或 Agent refinement 合同。它指导后续基于 `analysis.rules-fallback.yaml`、`analysis.multitrack.yaml` 和证据包改进 `analysis.yaml` / `report.md`，但当前不是自动闭环的生产推理核。

Phase 4 的 `analysis.yaml` 顶层契约字段由 `src/phases/phase4/analysis_contract.py` 统一生成，包括 `retrieval_context`、`experience_matches` 和 `source_boundaries`。这些字段用于未来历史经验/向量召回和来源边界治理；历史经验不能直接作为当前故障结论证据。
