...
def validate_executor_capabilities(
    ...
    *,
    ...
    if not which_fn("sshpass"):
        return False, [remote_capabilities.capability_result("sshpass", "blocked", "sshpass is not installed locally", "missing_sshpass")], {
    ...
    if ssh_proc.returncode != 0:
        code = remote_capabilities.classify_ssh_error(ssh_proc.stderr)
        checks.append(remote_capabilities.capability_result("ssh", "blocked", ssh_proc.stderr.strip() or "ssh check failed", code))
        return False, checks, {"code": code, "message": ssh_proc.stderr.strip() or "ssh check failed"}
    checks.append(remote_capabilities.capability_result("ssh", "success", "ssh echo ok succeeded"))

    kubectl_proc = run_ssh_fn(access, "kubectl version --client=true >/dev/null", 20)
    if kubectl_proc.returncode != 0:
        code = remote_capabilities.classify_kubectl_error(kubectl_proc.stderr)
        checks.append(remote_capabilities.capability_result("kubectl", "blocked", kubectl_proc.stderr.strip() or "kubectl client check failed", code))
        return False, checks, {"code": code, "message": kubectl_proc.stderr.strip() or "kubectl client check failed"}
    checks.append(remote_capabilities.capability_result("kubectl", "success", "kubectl client is available"))

    cluster_proc = run_ssh_fn(access, "kubectl get nodes -o name >/dev/null", 20)
    if cluster_proc.returncode != 0:
        code = remote_capabilities.classify_kubectl_error(cluster_proc.stderr)
        checks.append(remote_capabilities.capability_result("k8s_context", "blocked", cluster_proc.stderr.strip() or "kubectl cluster access check failed", code))
        return False, checks, {"code": code, "message": cluster_proc.stderr.strip() or "kubectl cluster access check failed"}
    checks.append(remote_capabilities.capability_result("k8s_context", "success", "kubectl can access the cluster"))

    exec_proc = run_ssh_fn(access, "kubectl auth can-i create pods/exec -A", 20)
    if exec_proc.returncode != 0:
        checks.append(
            remote_capabilities.capability_result(
                "kubectl_exec",
                "blocked",
    ...
    checks.append(remote_capabilities.capability_result("kubectl_exec", "success", "kubectl exec capability is available"))
