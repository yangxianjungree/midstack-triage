...
- `analysis.yaml.agent_conclusion_gate` 是 Agent 草稿提升资格评估；它检查 runtime、候选状态/置信度、当前 incident 证据引用、hypothesis-only source 边界和 unresolved critical gap。当前只记录 `decision` 与 blockers，不应用 override。
...
- 生产 `conclusion_summary` 当前仍由 rules fallback + guardrails 守底生成；`analysis.multitrack.yaml` 和 `analysis.yaml.agent_reasoning` 记录 Agent 推理辅助草稿，`agent_conclusion_gate` 记录该草稿是否具备提升资格
