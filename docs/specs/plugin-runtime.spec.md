...
| --- | --- | --- | --- |
| Phase 1 启动 | 创建或继续 incident、解析线索、保存用户上下文和环境模式 | `/<plugin_name>:start` | Agent 只抽取参数并调用 runtime；不自行排障 |
| Phase 2 readiness 与盘点 | 持续补齐输入、执行接入校验、namespace、对象、拓扑、auth hint | `/<plugin_name>:start` | 由 start runtime 内部完成，ready 后提示 analyse |
...
- `/plugin:start`
  - Phase 1 创建或继续记录并保存输入上下文
  - Phase 2 readiness gate 缺信息或校验失败进入 `blocked`
  - Phase 2 readiness gate 与对象盘点通过进入 `ready`
- `/plugin:analyse`
...
- `pasted_evidence`：`offline` 模式可选；保存为原始证据，不直接进入分析
...
- `offline` 模式提供完整 `artifact_source` 时，`/start` 可返回 `ready`，但不执行分析；下一步仍走 `/plugin:analyse`，由 incident 中的 `execution_mode: offline` 选择离线产物消费路径
...
- `incident_id`：可选
- `scope`：可选，默认 `full`；当前仅支持 `full`、`collect` 与 `reason`

规则：

- 无显式 `incident_id` 时，默认分析当前目标记录
- 如存在多个未结束 incident，优先命中当前会话最近目标记录
- `/analyse` 不接受用户传入的 `--execution-mode`；执行模式只从当前 incident 的 `input.yaml.execution_mode` / `input.yaml.environment_mode` 派生
...
- `scope=collect` 只执行第 3 段采集治理，产出 `input.yaml`、`structured_record.yaml`、`signal_bundle.yaml`、`collection_report.yaml` 后停止；不生成新的 `analysis.yaml` 或 `report.md`。如果目录中已有旧推理输出，运行时保留文件但在 `adapter-output.yaml.reasoning_outputs.status` 和 `meta.yaml.reasoning_outputs.status` 标记为 `stale`，提示后续运行 `scope=reason`
