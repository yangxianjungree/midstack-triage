...
- `local`
  - 表示 Agent/runtime 已经在故障集群或控制面机器上。显式 `local` 由 Phase 2 验证本机 `kubectl` context 与对象盘点；通过后写入 `local-config.yaml`，后续分析通过本地 transport 复用 Phase 3 脚本编排，不经过 SSH。
  - 当远程接入信息缺失或用户显式选择 `local` 时，Phase 1 可做轻量本地 `kubectl` context 探测，并把结果作为 `local_context` 提示；是否可进入 `ready` 仍由 Phase 2 readiness gate 决定。
- `offline`
...
- `local_fault_cluster`：runtime 已在故障集群或控制面机器上；Phase 2 gate 通过后可进入本地 live 采集路径。
...
- 临时只读命令是二等资产，必须使用 `asset.type: ad_hoc_command` 和结构化 `asset.argv`，不得使用 shell 字符串；只读 guardrail 通过后仍必须是 `execution_policy: approval_required`，不得自动执行。
